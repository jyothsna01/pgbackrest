...
sub storageRepo
{
    ...
    if (!defined($hStorage->{&STORAGE_REPO}{$strStanza}))
    {
        if (isRepoLocal())
        {
            ...
            # If the encryption is not the default (none) then set the user-defined key and Magic based on the type
            if (cfgOption(CFGOPT_REPO_CIPHER_TYPE) ne CFGOPTVAL_REPO_CIPHER_TYPE_NONE)
            {
                $strCipherType = cfgOption(CFGOPT_REPO_CIPHER_TYPE);
                $strCipherKey = cfgOption(CFGOPT_REPO_CIPHER_KEY);
            }
            ...
            $hStorage->{&STORAGE_REPO}{$strStanza} = new pgBackRest::Storage::Local(
                ...
                strCipherType => $strCipherType, strCipherKeyUser => $strCipherKey});
        }
        else
        {
