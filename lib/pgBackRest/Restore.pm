...
sub manifestLoad
{
    ...
    (
        $strOperation,
        $strCipherKey,                          # Key to access the manifest file if the repo is encrypted
    ) =
        logDebugParam
        (
            __PACKAGE__ . '->manifestLoad', \@_,
            {name => 'strCipherKey', required => false, redact => true},
        );
...
sub process
{
    ...
    my $oManifest = $self->manifestLoad($oBackupInfo->cipherKeySub());
        ...
        ($oManifest->keys(MANIFEST_SECTION_TARGET_FILE, INI_SORT_NONE)))
    {
        ...
        $oRestoreProcess->queueJob(
            ...
            {rParamSecure => $oManifest->cipherKeySub() ? [$oManifest->cipherKeySub()] : undef});
    }
