...
sub new
{
    ...
    (
        ...
        (
            ...
            {name => 'strMode', optional => true, default => STORAGE_ENCIPHER, trace => true},
        );
    ...
    if (!($self->{strMode} eq STORAGE_ENCIPHER || $self->{strMode} eq STORAGE_DECIPHER))
    {
    ...
    $self->{oCipher} = new pgBackRest::LibC::Cipher::Block(
        $self->{strMode} eq STORAGE_ENCIPHER ? CIPHER_MODE_ENCIPHER : CIPHER_MODE_DECIPHER, $strCipherType, $tCipherKey,
        length($tCipherKey));
