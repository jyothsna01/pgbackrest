...
sub new
{
    ...
    (
        ...
        (
            ...
            {name => 'strCipherKeyUser', optional => true, redact => true},
        );
    ...
    $self->{strCipherKeyUser} = $strCipherKeyUser;
    ...
    if (defined($oFileIo))
    {
        ...
        if (defined($self->cipherType()))
        {
            $oFileIo = &STORAGE_FILTER_CIPHER_BLOCK->new(
                $oFileIo, $self->cipherType(), defined($strCipherKey) ? $strCipherKey : $self->cipherKeyUser(),
...
####################################################################################################################################
# cipherKeyGen - generate a key of the specified size (in bytes)
####################################################################################################################################
sub cipherKeyGen
{
    my $self = shift;
    ...
    my $strCipherKey = encodeToStr(0, randomBytes($iKeySizeInBytes));

    # Return from function and log return values if any
    return logDebugReturn
    (
        $strOperation,
        {name => 'strCipherKeySub', value => $strCipherKey, redact => true}
    );
}
...
sub cipherKeyUser {shift->{strCipherKeyUser}}
