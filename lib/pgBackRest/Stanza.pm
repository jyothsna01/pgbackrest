...
sub stanzaCreate
{
    ...
    if (@stryFileListArchive || @stryFileListBackup)
    {
        ...
        {
            confess &log(ERROR, 'files exist - the encryption type or key cannot be changed', ERROR_PATH_NOT_EMPTY);
        }
        ...
        # have the key that was in the info file to open the other files in the repo)
...
sub infoObject
{
    ...
    eval
    {
        ...
        # else existed in the repo so a key is generated to store in the file. If it exists and the repo is encrypted then the
        # generated key passed will not be used - the one from the info file will be read.
        my $oParamRef =
            {bIgnoreMissing => $bIgnoreMissing,
             strCipherKeySub => defined(storageRepo()->cipherType()) ?
                storageRepo()->cipherKeyGen() : undef};
