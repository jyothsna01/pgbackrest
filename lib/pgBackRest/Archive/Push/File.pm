...
sub archivePushCheck
{
    ...
    {
        # Execute the command
        ($strArchiveId, $strChecksum, $strCipherKey) = protocolGet(CFGOPTVAL_REMOTE_TYPE_BACKUP)->cmdExecute(
    ...
    else
    {
        ...
        # Get the encryption key to read/write files (undefined if the repo is not encrypted)
    ...
    (
        ...
        {name => 'strCipherKey', value => $strCipherKey, redact => true},
...
sub archivePushFile
{
    ...
    my ($strArchiveId, $strChecksum, $strCipherKey, $strWarning) = archivePushCheck(
