...
sub new
{
    ...
    (
        ...
        $strCipherKeySub,                       # Encryption key to encrypt the subsequent archive files if repo is encrypted
    ) =
        logDebugParam
        (
            __PACKAGE__ . '->new', \@_,
            ...
            {name => 'strCipherKeySub', optional => true},
        );
    ...
    eval
    {
        $self = $class->SUPER::new($strArchiveInfoFile, {bLoad => $bLoad, bIgnoreMissing => $bIgnoreMissing,
            oStorage => storageRepo(), strCipherKey => storageRepo()->cipherKeyUser(),
...
sub reconstruct
{
    ...
    foreach my $iDbHistoryId (sort {$a <=> $b} keys %hDbHistoryVersion)
    {
        ...
        # If the file is encrypted, then the key from the info file is required, else getEncryptionKeySub returns undefined
        my $oFileIo = storageRepo()->openRead(
            ...
            strCipherKey => $self->cipherKeySub()});
