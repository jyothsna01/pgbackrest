...
sub processManifest
{
        ...
        ($oBackupManifest->keys(MANIFEST_SECTION_TARGET_FILE, INI_SORT_NONE)))
    {
        ...
        $oBackupProcess->queueJob(
            ...
            {rParamSecure => $oBackupManifest->cipherKeySub() ? [$oBackupManifest->cipherKeySub()] : undef});
...
sub process
{
    ...
    # Get encryption key to open manifest (undefined if repo not encrypted) and intialize key variable for backup files
    my $strCipherKeyManifest = $oBackupInfo->cipherKeySub();
    my $strCipherKeyBackupSet;
    ...
    if ($strType ne CFGOPTVAL_BACKUP_TYPE_FULL)
    {
        ...
        if (defined($strBackupLastPath) && $oBackupInfo->confirmDb($strBackupLastPath, $strDbVersion, $ullDbSysId))
        {
            $oLastManifest = new pgBackRest::Manifest(
                ...
                {strCipherKey => $strCipherKeyManifest});

            # If the repo is encrypted then use the key in this manifest for the backup set
            $strCipherKeyBackupSet = $oLastManifest->cipherKeySub();
        ...
        STORAGE_REPO_BACKUP, {strExpression => backupRegExpGet(true, true, true), strSortOrder => 'reverse'}))
    {
            ...
            !$oStorageRepo->exists(STORAGE_REPO_BACKUP . "/${strAbortedBackup}/" . FILE_MANIFEST))
        {
            ...
            if ($bUsable)
            {
                ...
                # If the repo is encrypted, set the backup set encryption key from this manifest
                if (defined($strCipherKeyManifest))
                {
                    $strCipherKeyBackupSet = $oAbortedManifest->cipherKeySub();
                }
            }
            else
            {
    ...
    # Generate a key for the backup set if the repo is encrypted
    if (defined($strCipherKeyManifest) && !defined($strCipherKeyBackupSet) && $strType eq CFGOPTVAL_BACKUP_TYPE_FULL)
    {
        $strCipherKeyBackupSet = $oStorageRepo->cipherKeyGen();
    }
    ...
    # Instead just instantiate it. Pass the keys to open the manifest and one to encrypt the backup files if the repo is
    ...
    my $oBackupManifest = new pgBackRest::Manifest("$strBackupPath/" . FILE_MANIFEST,
        ...
        strCipherKey => defined($strCipherKeyManifest) ? $strCipherKeyManifest : undef,
        strCipherKeySub => defined($strCipherKeyManifest) ? $strCipherKeyBackupSet : undef});
    ...
    if (cfgOption(CFGOPT_ONLINE))
    {
        ...
        foreach my $strFile (sort(keys(%{$oFileHash})))
        {
            ...
            if (defined($oFileHash->{$strFile}))
            {
                ...
                # If the backups are encrypted, then the key for the backup set from the manifest file is required to access the
                # file in the repo
                my $oDestinationFileIo = $oStorageRepo->openWrite(
                    STORAGE_REPO_BACKUP . "/${strBackupLabel}/${strFile}" . ($bCompress ? qw{.} . COMPRESS_EXT : ''),
                    {rhyFilter => $rhyFilter,
                    strCipherKey => defined($strCipherKeyBackupSet) ? $strCipherKeyBackupSet : undef});
            ...
            if (cfgOption(CFGOPT_ARCHIVE_COPY))
            {
                ...
                $oStorageRepo->copy(
                    $oStorageRepo->openRead(STORAGE_REPO_ARCHIVE . "/${strArchiveId}/${strArchiveFile}",
                        {strCipherKey => $oArchiveInfo->cipherKeySub()}),
                    $oStorageRepo->openWrite(STORAGE_REPO_BACKUP . "/${strBackupLabel}/" . MANIFEST_TARGET_PGDATA . qw{/} .
                        ...
                        {strCipherKey => $strCipherKeyBackupSet})
                    );
    ...
    # Copy a compressed version of the manifest to history. If the repo is encrypted then the key to open the manifest is required.
    $oStorageRepo->copy(
