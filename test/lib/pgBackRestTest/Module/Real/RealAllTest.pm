...
sub run
{
        ...
        $bS3 || $bHostBackup ? (HOST_BACKUP) : $bHostStandby ? (HOST_DB_MASTER, HOST_DB_STANDBY) : (HOST_DB_MASTER))
    {
        ...
        # Get encryption key to access the Manifest file from backup.info - returns undefined if repo not encrypted
        my $strCipherKey =
            (new pgBackRest::Backup::Info(storageRepo()->pathGet(STORAGE_REPO_BACKUP)))->cipherKeySub();
        ...
        my $oManifest = new pgBackRest::Manifest(BOGUS, {bLoad => false, strDbVersion => $self->pgVersion(),
            strCipherKey => $strCipherKey, strCipherKeySub => $bRepoEncrypt ? ENCRYPTION_KEY_BACKUPSET : undef});
...
# CSHANG May need to revist some of these for encryption
            if (!$bRepoEncrypt)
            {
