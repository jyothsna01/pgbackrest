...
sub run
{
    ...
    if ($self->begin('encryption'))
    {
        ...
        # Error when encryption key not passed
        #---------------------------------------------------------------------------------------------------------------------------
        my $oFileIo = $self->testException(sub {$self->storageEncrypt()->openWrite($strFile)},
            ERROR_ASSERT, 'tCipherKey is required in Storage::Filter::CipherBlock->new');

        # Write an encrypted file
        #---------------------------------------------------------------------------------------------------------------------------
        $oFileIo = $self->testResult(sub {$self->storageEncrypt()->openWrite($strFile, {strCipherKey => $strCipherKey})},
            '[object]', 'open write');
        ...
        #---------------------------------------------------------------------------------------------------------------------------
        $oFileIo = $self->testResult(sub {$self->storageEncrypt()->openRead($strFile, {strCipherKey => $strCipherKey})},
        ...
        $self->testResult(
            sub {$self->storageEncrypt()->copy(
                $self->storageEncrypt()->openRead($strFile, {strCipherKey => $strCipherKey}),
                $self->storageEncrypt()->openWrite($strFileCopy, {strCipherKey => $strCipherKey}))},
            true, 'copy - decrypt/encrypt');

        $self->testResult(
            sub {sha1_hex(${$self->storageEncrypt()->get($strFileCopy, {strCipherKey => $strCipherKey})})}, $strFileHash,
        ...
        my $strZeroContent = '';
        $oFileIo = $self->testResult(
            sub {$self->storageEncrypt()->openWrite($strFileZero, {strCipherKey => $strCipherKey})}, '[object]',
            'open write for zero');
