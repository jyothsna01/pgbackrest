        ...
        $oExpectedManifest = iniParse(
            ${storageRepo()->get(
                storageRepo()->openRead(
                    ...
                    {strCipherKey => $self->cipherKeyManifest()}))});
    }
...
sub backupCompare
{
    ...
    if (defined($oExpectedManifest->{&INI_SECTION_CIPHER}) &&
        defined($oExpectedManifest->{&INI_SECTION_CIPHER}{&INI_KEY_CIPHER_KEY}) &&
        $oActualManifest->test(INI_SECTION_CIPHER, INI_KEY_CIPHER_KEY))
    {
        $oExpectedManifest->{&INI_SECTION_CIPHER}{&INI_KEY_CIPHER_KEY} =
            $oActualManifest->get(INI_SECTION_CIPHER, INI_KEY_CIPHER_KEY);
    }
...
sub stanzaCreate
{
    ...
    if (storageRepo()->exists('backup/' . $self->stanza() . qw{/} . FILE_BACKUP_INFO))
    {
        ...
        # Get the encryption key for accessing the manifest file
        $self->{strCipherKeyManifest} =
            (new pgBackRest::Backup::Info(storageRepo()->pathGet('backup/' . $self->stanza())))->cipherKeySub();
    }
    ...
    if (storageRepo()->exists('archive/' . $self->stanza() . qw{/} . ARCHIVE_INFO_FILE))
    {
        ...
        # Get the encryption key for accessing the archived files
        $self->{strCipherKeyArchive} =
            (new pgBackRest::Archive::Info(storageRepo()->pathGet('archive/' . $self->stanza())))->cipherKeySub();
    }
...
sub configCreate
{
    ...
    if ($self->isHostBackup())
    {
        if ($self->repoEncrypt())
        {
            ...
            $oParamHash{&CFGDEF_SECTION_GLOBAL}{cfgOptionName(CFGOPT_REPO_CIPHER_KEY)} = 'x';
        }
...
sub infoMunge
{
    ...
    if (!defined($self->{hInfoFile}{$strFileName}))
    {
        ...
        strCipherKey => !$bManifest ? storageRepo()->cipherKeyUser() : $self->cipherKeyManifest()});
    }
...
sub cipherKeyManifest {return shift->{strCipherKeyManifest}}
sub cipherKeyArchive {return shift->{strCipherKeyArchive}}
